...
function clearGuest() {
  ['guestToken', 'user', 'guestId'].forEach(key => localStorage.removeItem(key));
}
...
function searchPhotos(keyword = '', category = '', sort = '') {
  const params = new URLSearchParams({ ...(keyword && { keyword }), ...(category && { category }), ...(sort && { sort }) });
