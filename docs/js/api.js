...
async function apiCall(endpoint, method = 'GET', body = null, useToken = true) {
  ...
  const response = await fetch(`${API_BASE}${endpoint}`, { method, headers, body: body instanceof FormData ? body : body ? JSON.stringify(body) : undefined });
  const type = response.headers.get('content-type') || ''; const data = type.includes('application/json') ? await response.json() : {};
...
function getPhoto(id) { return apiCall(`/photos/${id}`, 'GET', null, false); } function getComments(id) { return apiCall(`/comments/photo/${id}`, 'GET', null, false); } function addComment(photoId, content) { return apiCall('/comments', 'POST', { photoId, content }); } function ratePhoto(photoId, score) { return apiCall('/ratings', 'POST', { photoId, score }); } function removeComment(id) { return apiCall(`/comments/${id}`, 'DELETE'); } function uploadPhoto(formData) { return apiCall('/photos/upload', 'POST', formData); } function downloadPhoto(id) { window.location.href = `${API_BASE}/photos/${id}/download`; }
